...
'''
import sys
sys.path.append('/home/kali/peCloakCapstone')
		...
		for byte in data_to_search:
			...
			if byte == "00":
				...
				if null_count >= required_space: # we've hit our required space limit
					...
					print("Calculated value: ", (raw_offset) + virtual_address - pe.OPTIONAL_HEADER.AddressOfEntryPoint)

					virtual_offset = struct.pack("=Q",(raw_offset) + virtual_address - pe.OPTIONAL_HEADER.AddressOfEntryPoint)
...
def get_code_cave (pe, skip_cave_search):
	...
	code_cave_address = hex(pe.OPTIONAL_HEADER.ImageBase + pe.OPTIONAL_HEADER.AddressOfEntryPoint + struct.unpack("=Q",code_cave_virtual_offset)[0])
...
def get_virtual_offset(virtual_address, pe):
	...
	return struct.pack("=Q",virtual_address - pe.OPTIONAL_HEADER.AddressOfEntryPoint)	
		...
		if len(sections) > 0:
			for section in sections:	
				...
				if section:
					...
					# Diagnostics: Print values for debugging
					print("section_start: ", section_start)
					print("encode_offset: ", encode_offset)
					print("Combined value for decode_start: ", section_start + encode_offset)

					# # Check if the combined value for decode_start is within the valid range
					# combined_value_start = section_start + encode_offset
					# if not (0 <= combined_value_start <= 4294967295):
					#     print("Combined value for decode_start is out of valid range for 'L' format.")
					#     continue  # Skip this section or handle error as needed

					try:
					    # decode_start = struct.pack("=L", combined_value_start)
					    decode_start = struct.pack("=Q", section_start + encode_offset)
					except struct.error as e:
					    print("Struct Error for decode_start: ", e)
					    continue  # Skip this section or handle error as needed

					# # Repeat the diagnostic steps for decode_end
					# print("Combined value for decode_end: ", section_start + encode_offset + encode_length)
					# combined_value_end = section_start + encode_offset + encode_length
					# if not (0 <= combined_value_end <= 4294967295):
					#     print("Combined value for decode_end is out of valid range for 'L' format.")
					#     continue  # Skip this section or handle error as needed

					try:
					    # decode_end = struct.pack("=L", combined_value_end)
					    decode_end = struct.pack("=Q", section_start + encode_offset + encode_length)
					except struct.error as e:
					    print("Struct Error for decode_end: ", e)
					    continue  # Skip this section or handle error as needed
					...
					if encode_length == 0:
						...
						decode_end = struct.pack("=Q", section_start + section_header.Misc_VirtualSize)
					else:
