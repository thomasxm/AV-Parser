...
class FileLabeler:
    ...
    def get_sample_info_lb(vt_rep):
        ...
        md5 = vt_rep.get('md5', '')  # Default to empty string if 'md5' key is missing
        sha1 = vt_rep.get('sha1', '')  # Default to empty string if 'sha1' key is missing
        sha256 = vt_rep.get('sha256', '')  # Default to empty string if 'sha256' key is missing
        return SampleInfo(md5, sha1, sha256, vt_rep['av_labels'], [])
    ...
    def process_line(self, line, get_sample_info):
        ...
        if 'first_seen' in vt_rep:
            del vt_rep['first_seen']
        if 'sha256' in vt_rep:
            del vt_rep['sha256']
        if 'md5' in vt_rep:
            del vt_rep['md5']
